from eval_varcalls.get_induced_ref_stats import remove_indels,remove_read_ends,majority_pileup_is_non_ref,get_fraction_below_depth

class TestFractionBelowDepth:
    def test_all_zerodepth(self):
        depths = [0, 0]
        assert get_fraction_below_depth(depths, 0) == 1
        assert get_fraction_below_depth(depths, 1) == 1

    def test_mix_of_depths(self):
        depths = [0,1,2,3]
        assert get_fraction_below_depth(depths, 0) == 0.25
        assert get_fraction_below_depth(depths, 2) == 0.75
